...
void clockCounter_Start(void) 
{
    ...
    clockCounter_CLKEN |= clockCounter_CLKEN_MASK;
	clockCounter_CLKSTBY |= clockCounter_CLKSTBY_MASK;
}
...
void clockCounter_Stop(void) 
{
    ...
    clockCounter_CLKEN &= (uint8)(~clockCounter_CLKEN_MASK);
	clockCounter_CLKSTBY &= (uint8)(~clockCounter_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: clockCounter_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((clockCounter_CLKEN & clockCounter_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clockCounter_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clockCounter_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* clockCounter__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(clockCounter_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clockCounter_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clockCounter_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clockCounter_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clockCounter_GetSourceRegister();
    uint16 oldDivider = clockCounter_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clockCounter_CLKEN & clockCounter_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(clockCounter_DIV_PTR, clkDivider);
                clockCounter_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                clockCounter_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(clockCounter_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clockCounter_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clockCounter_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clockCounter_DIV_PTR);
}


/*******************************************************************************
* Function Name: clockCounter_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clockCounter_MOD_SRC |= modeBitMask & (uint8)clockCounter_MODE_MASK;
}


/*******************************************************************************
* Function Name: clockCounter_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clockCounter_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clockCounter_MODE_MASK));
}


/*******************************************************************************
* Function Name: clockCounter_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clockCounter_MOD_SRC & (uint8)(clockCounter_MODE_MASK);
}


/*******************************************************************************
* Function Name: clockCounter_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clockCounter_GetDividerRegister();
    uint8 oldSrc = clockCounter_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clockCounter_MOD_SRC |= CYCLK_SSS;
        clockCounter_MOD_SRC =
            (clockCounter_MOD_SRC & (uint8)(~clockCounter_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clockCounter_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clockCounter_MOD_SRC & clockCounter_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clockCounter_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clockCounter_PHASE = clkPhase & clockCounter_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clockCounter_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clockCounter_PHASE & clockCounter_PHASE_MASK;
}
