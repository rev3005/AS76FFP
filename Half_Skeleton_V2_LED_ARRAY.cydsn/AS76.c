...
{  
    ...
    Encoder_Position_X_Requested = steps/3.2;
...
{  
    ...
    //int32 diff_encoder_value=0;
    int32 encoder_error_value = 0;
    Encoder_Position_Y_Requested = steps/3.2;
...
int32 z_microns=409;
 CY_ISR(Quad_Timer_Z)
{    
    Z_QuadPosition = (int32)((float)((float)-QuadDec_TZ_GetCounter()/16)*51.2);
...
void Process_USB_Data()/* Process USB incoming data command. */
{
    ...
    else if (command == Set_LED_Array)
    { 
        if(USB_received[2] == 0)
        {
            BF(USB_received[4], USB_received[6], USB_received[8], USB_received[10], USB_received[12]);
        }
        else if(USB_received[2] == 1)
        {
            Ring(USB_received[4], USB_received[6], USB_received[8], USB_received[10], USB_received[12]);    
        }
        else if(USB_received[2] == 3)
        {
            PCA9959_ChnEn(USB_received[4], USB_received[6]);       
        } 
        else
        {
...
void Initialize_Motor(uint8_t Motor)
{
            ...
            Write_32bitSPI_DATA (0x10  , (int) 0x00070303, Motor );
            }
