...
{
    Write_32bitSPI_DATA (0x10  , (int) 0x00070503, TMC5160_nCS_MotorX );
    ...
    CyDelayUs(500);
    Read_32bitSPI_DATA(0x04, Motor, &status_x); // Read the status register of TMC5160
    Write_Debug_UART_Char("Limit Switch X going to press \n");
    ...
    wait_timer_Stop();
    exit_loop =0;
    Write_Debug_UART_Char("Limit Switch X Pressed \n");
...
{
    Write_32bitSPI_DATA (0x10  , (int) 0x00070203, TMC5160_nCS_MotorY );
    ...
    CyDelayUs(500);
    Read_32bitSPI_DATA(0x04, Motor, &status_y);
    Write_Debug_UART_Char("Limit Switch Y going to press \n");
    ...
    wait_timer_Stop();
    exit_loop =0;
    Write_Debug_UART_Char("Limit Switch Y Pressed \n");
    ...
    */
    ...
    //goTo_Y((256*200)*1);
    goTo_Y(19200);
    Write_Debug_UART_Char("Error in Home Y Before Goto home pos : ");
    Write_Debug_UART_Int(Error);
    Write_Debug_UART_Char("\n");
    HomeX_done = HomeX_done_Buffer;
    HomeY_done = HomeY_done_Buffer;
...
void homeZ(uint8_t Motor)
{
    ...
    CyDelayUs(500);
    Read_32bitSPI_DATA(0x04, Motor, &status_z);
    Write_Debug_UART_Char("Limit Switch Z going to press \n");
    ...
    wait_timer_Stop();
    exit_loop =0;
    Write_Debug_UART_Char("Limit Switch Z Pressed \n");
    ...
    */
    ...
    CyDelayUs(100);
    goTo_Z(((256*200)*1));
    Write_Debug_UART_Char("Error in Home Z Before Goto home pos : ");
    Write_Debug_UART_Int(Error);
    Write_Debug_UART_Char("\n");
    Error =0;
    HomeX_done = HomeX_done_Buffer;
...
{  
    ...
    Encoder_Position_X_Requested = steps/X_Pitchfactor;
      Read_All_Optical_Encoder();    
      Write_Debug_UART_Char("Initial Error X Quadposition: ");
      Write_Debug_UART_Int(X_QuadPosition);
      Write_Debug_UART_Char("\n");
    
    for(temp=0; temp<=5; temp++)
    {
    Write_Debug_UART_Char("X Loop Count: ");
    Write_Debug_UART_Int(temp);
    Write_Debug_UART_Char("\n");
...
{   
    if(if_all_homing_done())
    {
        Write_32bitSPI_DATA (0x10  , (int) 0x00070103, TMC5160_nCS_MotorX );
        Write_32bitSPI_DATA (0x10  , (int) 0x00070102, TMC5160_nCS_MotorZ );
        Write_32bitSPI_DATA (0x10  , (int) 0x00070305, TMC5160_nCS_MotorY );
...
{  
    ...
    Read_All_Optical_Encoder();
    Write_Debug_UART_Char("Initial Error Y Quadposition: ");
    Write_Debug_UART_Int(Y_QuadPosition);
    Write_Debug_UART_Char("/n");
    
    for(temp=0; temp<=5; temp++)
    {
    Write_Debug_UART_Char("Y Loop Count: ");
    Write_Debug_UART_Int(temp);
    Write_Debug_UART_Char("\n");
    Read_All_Optical_Encoder();
    ...
    if(temp>=4)
    {
     if(((abs(encoder_error_value))>5) && ((abs(encoder_error_value))<2000) )
     {
...
{  
    ...
    if(temp>=4)
    {
     ...
     {
        Write_Debug_UART_Char("Fail to reach Z position, Error difference: ");
        Write_Debug_UART_Int(encoder_error_value);
        Write_Debug_UART_Char("\n Quadposition :  ");
        Write_Debug_UART_Int(Z_QuadPosition);
        Write_Debug_UART_Char("\n");
...
void Process_USB_Data()/* Process USB incoming data command. */
{
    ...
    if (command == HomeX)                                          
    {
        ...
        //Write_32bitSPI_DATA (0x26  , (int) 45000, TMC5160_nCS_MotorX );
        CyDelay(1);
        Input_Position_X_Temp = 0;
        goTo_X(0);
    ...
    else if (command == HomeY)                           
    {
        ...
        TMC5160_MotorY_EN_Write(0x00);
        CyDelayUs(500);
        //Write_32bitSPI_DATA (0x26  , (int) 45000, TMC5160_nCS_MotorY );
        ...
        Write_32bitSPI_DATA (0x10  , (int) 0x00070301, TMC5160_nCS_MotorY );
        CyDelay(50);
        TMC5160_MotorZ_EN_Write(0x00);
        CyDelay(50);
        TMC5160_MotorX_EN_Write(0x00);
    ...
    else if (command == HomeZ)                           
    {
        ...
        TMC5160_MotorZ_EN_Write(0x00);
        CyDelayUs(500);
        //Write_32bitSPI_DATA (0x26  , (int) 45000, TMC5160_nCS_MotorZ );
        update_max_velocity(53687*2, TMC5160_nCS_MotorZ);
        CyDelay(1);
        goTo_Z(0);
        Write_32bitSPI_DATA (0x10  , (int) 0x00070101, TMC5160_nCS_MotorY );
        Write_32bitSPI_DATA (0x10  , (int) 0x00070303, TMC5160_nCS_MotorZ );
        Write_32bitSPI_DATA (0x10  , (int) 0x00070301, TMC5160_nCS_MotorX );
    ...
    else if (command == Set_Backlight)
    {
        ...
        {
        Write_Debug_UART_Char("Backlight LED wrong Value Provided  \r\n");
        }
     ...
     else if (command == Strip_LED)                          
    {
        ...
        BF22(R1,G1,B1,MB1,R2,G2,B2,MB2,R3,G3,B3,MB3,R4,G4,B4,MB4,R5,G5,B5,MB5,R6,G6,B6,MB6);
    ...
    else if (command == GSV2_1)
    {
        ...
        GsV2_1( Xstart, Xstop,  deltaz,  Motor_Speed_X,  Motor_Speed_Z, Z_Point1, Z_Point2) ;
    ...
    else if (command == GSV2_3)
    {
        ...
        GsV2_2( Xstart, Xstop,  deltaz,  Motor_Speed_X,  Motor_Speed_Z, Z_Point1, Z_Point2) ;
     ...
     else if (command == GSV2LR)
    {
        
        int X1 ,X2,X3,X4,Y1,Y2,Y3,Y4,deltaz  =0;
        X1 = ((int32)USB_received[5] << 24) + ((int32)USB_received[4] << 16) + ((int32)USB_received[3] << 8) + (int32)USB_received[2];
        X2 = ((int32)USB_received[9] << 24) + ((int32)USB_received[8] << 16) + ((int32)USB_received[7] << 8) + (int32)USB_received[6];
        X3 = ((int32)USB_received[13] << 24) + ((int32)USB_received[12] << 16) + ((int32)USB_received[11] << 8) + (int32)USB_received[10];        
        X4 = ((int32)USB_received[17] << 24) + ((int32)USB_received[16] << 16) + ((int32)USB_received[15] << 8) + (int32)USB_received[14];
        Y1 = ((int32)USB_received[21] << 24) + ((int32)USB_received[20] << 16) + ((int32)USB_received[19] << 8) + (int32)USB_received[18];
        Y2 = ((int32)USB_received[25] << 24) + ((int32)USB_received[24] << 16) + ((int32)USB_received[23] << 8) + (int32)USB_received[22];
        Y3 = ((int32)USB_received[29] << 24) + ((int32)USB_received[28] << 16) + ((int32)USB_received[27] << 8) + (int32)USB_received[26];
        Y4 = ((int32)USB_received[33] << 24) + ((int32)USB_received[32] << 16) + ((int32)USB_received[31] << 8) + (int32)USB_received[30];
        
        deltaz = ((int32)USB_received[37] << 24) + ((int32)USB_received[36] << 16) + ((int32)USB_received[35] << 8) + (int32)USB_received[34];
        
        Motor_Speed_X = ((int32)USB_received[41] << 24) + ((int32)USB_received[40] << 16) + ((int32)USB_received[39] << 8) + (int32)USB_received[38];
        Motor_Speed_Z = ((int32)USB_received[45] << 24) + ((int32)USB_received[44] << 16) + ((int32)USB_received[43] << 8) + (int32)USB_received[42];
        ...
        GsV2_LR( X1,  X2,  X3,  X4 , Y1,  Y2, Y3,  Y4,  deltaz,  Motor_Speed_X,  Motor_Speed_Z);
        
        Previous_Z_Position = Y4+deltaz;
        Previous_X_Position = X4;
        Send_Feedback_to_USB(0);
    }
...
{
         //Write_Debug_UART_Char("Waiting for target position to reach\n");
...
void GsV2_LR(int X1, int X2, int X3, int X4 ,int Y1, int Y2,int Y3, int Y4, int deltaz, int Xspeed, int Zspeed)
{
    LED3_Write(0xFF);
    Enable_Encoder_Z(-Buffer_Z_QuadPosition);
    float a,b=0;    
    ...
    bnum = ((X1-Xavg) * (Y1-Yavg)) + ((X2-Xavg) * (Y2-Yavg)) +((X3-Xavg) * (Y3-Yavg)) +((X4-Xavg) * (Y4-Yavg));
    bdenom = (pow((X1-Xavg),2)) + (pow((X2-Xavg),2)) + (pow((X3-Xavg),2))+(pow((X4-Xavg),2));
    b = bnum/bdenom;
    a = Yavg - b*Xavg;
    
    
    tempz = (Y1 + (int)(deltaz/2));    
    goTo_XYZ(-1 ,X1,(Y1- (int)(deltaz/2))); 
    
    update_max_velocity(Xspeed, TMC5160_nCS_MotorY);
    update_max_velocity(Zspeed, TMC5160_nCS_MotorZ);
    
    start_quad = (X1/3.2)+72; 
    
    wait_timer_Start();
    wait_interrupt_StartEx(Quad_Timer1);
    
    GotoPos(X4, TMC5160_nCS_MotorY);
    GotoPos(tempz, TMC5160_nCS_MotorZ);
            ...
            if((Z_MOT_INT_Read()) == 0x00)
             {
                ...
                m = (((float)Z_Point2-(float)Z_Point1)/((float)EndPosX-(float)StartPosX));
                //Y_QuadPosition = -QuadDec_Y_GetCounter();
                k =  (Z_Point1 + m*(((((y_array/12.8)+(gsv2_microns*3))*12.8))-StartPosX));
                Z_QuadPosition = tempz-((-QuadDec_TZ_GetCounter())*12.8);
                tempz =  ((int32)(k + (pow(-1,n) * (deltaz/2))) + Z_QuadPosition );    
                GotoPos(tempz,TMC5160_nCS_MotorZ);
                n++;
                
                y_array = y_array + (gsv2_microns);
             }
        }
