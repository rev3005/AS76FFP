...
/*******************************************************************************
* Function Name: Clock_SPIM_Start
********************************************************************************
*
* Summary:
...
void Clock_SPIM_Start(void) 
{
    ...
    Clock_SPIM_CLKEN |= Clock_SPIM_CLKEN_MASK;
	Clock_SPIM_CLKSTBY |= Clock_SPIM_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_SPIM_Stop
********************************************************************************
*
* Summary:
...
void Clock_SPIM_Stop(void) 
{
    ...
    Clock_SPIM_CLKEN &= (uint8)(~Clock_SPIM_CLKEN_MASK);
	Clock_SPIM_CLKSTBY &= (uint8)(~Clock_SPIM_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_SPIM_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_SPIM_CLKEN & Clock_SPIM_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_SPIM_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_SPIM_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_SPIM__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_SPIM_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_SPIM_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_SPIM_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_SPIM_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_SPIM_GetSourceRegister();
    uint16 oldDivider = Clock_SPIM_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_SPIM_CLKEN & Clock_SPIM_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_SPIM_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_SPIM_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_SPIM_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_SPIM_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SPIM_MOD_SRC |= modeBitMask & (uint8)Clock_SPIM_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_SPIM_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SPIM_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_SPIM_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_SPIM_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_SPIM_MOD_SRC & (uint8)(Clock_SPIM_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_SPIM_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_SPIM_GetDividerRegister();
    uint8 oldSrc = Clock_SPIM_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_SPIM_MOD_SRC |= CYCLK_SSS;
        Clock_SPIM_MOD_SRC =
            (Clock_SPIM_MOD_SRC & (uint8)(~Clock_SPIM_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_SPIM_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_SPIM_MOD_SRC & Clock_SPIM_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_SPIM_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_SPIM_PHASE = clkPhase & Clock_SPIM_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_SPIM_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_SPIM_PHASE & Clock_SPIM_PHASE_MASK;
}
